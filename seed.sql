-- ============================================================
-- TaskFlow AI — Seed Data
-- Run this AFTER your CREATE TABLE script
-- Passwords are all: Password@123  (bcrypt hash below)
-- ============================================================

USE task_manager;

-- ============================================================
-- USERS  (1 admin + 7 members)
-- All passwords = Password@123
-- ============================================================
INSERT INTO users (name, email, password, role, avatar_color) VALUES
('Anshuman Raj',      'admin@taskflow.com',   '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.ucrm3zW5G', 'admin',  '#7c6df0'),
('Priya Sharma',      'priya@taskflow.com',   '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.ucrm3zW5G', 'member', '#8b5cf6'),
('Rahul Verma',       'rahul@taskflow.com',   '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.ucrm3zW5G', 'member', '#06b6d4'),
('Sneha Patel',       'sneha@taskflow.com',   '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.ucrm3zW5G', 'member', '#10b981'),
('Arjun Mehta',       'arjun@taskflow.com',   '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.ucrm3zW5G', 'member', '#f59e0b'),
('Kavya Nair',        'kavya@taskflow.com',   '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.ucrm3zW5G', 'member', '#ef4444'),
('Dev Khanna',        'dev@taskflow.com',     '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.ucrm3zW5G', 'member', '#ec4899'),
('Tanya Singh',       'tanya@taskflow.com',   '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.ucrm3zW5G', 'member', '#3b82f6');

-- ============================================================
-- PROJECTS  (6 projects, mixed statuses)
-- ============================================================
...
-- ============================================================
-- TASKS — Project 1: E-Commerce Revamp (20 tasks)
-- ============================================================
...
('Build cart & checkout flow v3',           'Multi-step checkout: cart → address → payment → confirmation. Mobile first.',       1, 4, 1, 'in_progress', 'critical', '2026-05-20', 'critical'),
...
('Customer review & ratings system',      'Star ratings, text reviews, verified purchase badge, helpful votes.',               1, 5, 2, 'todo',        'medium',   '2026-06-15', 'medium'),
('Wishlist & save-for-later feature',     'Persistent wishlist tied to account. Share wishlist via link.',                     1, 2, 1, 'todo',        'low',      '2026-06-20', 'low'),
...
('Coupon & discount management',          'Admin UI to create % and flat discounts, usage limits, expiry dates.',             1, 5, 1, 'todo',        'medium',   '2026-06-12', 'medium'),
('Cookie consent & GDPR compliance',      'Implement consent banner, preference centre, and data deletion request flow.',     1, 2, 1, 'done',        'high',     '2026-04-25', 'high'),
('Product variant management (size/color)','Handle SKU matrix: colour × size. Inventory per variant. Dynamic price.',        1, 4, 2, 'in_progress', 'critical', '2026-05-22', 'critical'),
...
-- ============================================================
-- TASKS — Project 2: Mobile App v2.0 (15 tasks)
-- ============================================================
INSERT INTO tasks (title, description, project_id, assigned_to, created_by, status, priority, due_date, ai_suggested_priority) VALUES
...
('Push notifications (FCM + APNs)',       'Real-time notifications for task assignments, comments, deadline reminders.',      2, 7, 1, 'in_progress', 'critical', '2026-05-15', 'critical'),
...
('Deep linking & universal links',        'Handle app:// scheme and https:// universal links. Route to correct screen.',     2, 7, 1, 'todo',        'high',     '2026-06-01', 'high'),
('Submit to App Store & Play Store',      'Prepare screenshots, descriptions, age ratings. Submit for review.',              2, 3, 1, 'todo',        'critical', '2026-07-01', 'critical'),
('Performance profiling & memory leaks',  'Use Flipper + Hermes profiler. Fix 3 identified memory leaks in FlatList.',       2, 8, 1, 'in_progress', 'high',     '2026-06-10', 'high'),
...
-- ============================================================
-- TASKS — Project 3: AI Analytics Dashboard (12 tasks)
-- ============================================================
INSERT INTO tasks (title, description, project_id, assigned_to, created_by, status, priority, due_date, ai_suggested_priority) VALUES
('Data pipeline: Kafka → ClickHouse',     'Stream events from Kafka topics into ClickHouse. Lag < 5 seconds.',               3, 4, 2, 'done',        'critical', '2026-04-12', 'critical'),
...
('CSV & PDF export for reports',         'Export any widget data as CSV. Full dashboard snapshot as branded PDF.',         3, 2, 2, 'todo',        'medium',   '2026-06-10', 'medium'),
('Anomaly detection alerts',             'Z-score based anomaly detection. Slack + email alert when metric spikes ±2σ.',   3, 4, 2, 'todo',        'high',     '2026-06-08', 'high'),
...
('Natural language query (NLQ)',         'Type "show me revenue last 30 days" → auto-generate SQL → render chart.',        3, 2, 2, 'todo',        'critical', '2026-07-01', 'critical'),
...
('Dark / light theme toggle',            'Persist theme preference per user in their profile settings.',                   3, 2, 2, 'done',        'low',      '2026-04-30', 'low');

-- ============================================================
-- TASKS — Project 4: DevOps Migration (10 tasks)
-- ============================================================
INSERT INTO tasks (title, description, project_id, assigned_to, created_by, status, priority, due_date, ai_suggested_priority) VALUES
...
('Set up Prometheus + Grafana',          'Scrape all pods. Create dashboards for CPU, memory, request rate, error rate.',  4, 7, 1, 'todo',        'high',     '2026-05-18', 'high'),
('Centralised logging with ELK stack',  'Filebeat → Logstash → Elasticsearch → Kibana. Retain 30 days.',                  4, 3, 1, 'todo',        'medium',   '2026-05-20', 'medium'),
...
-- ============================================================
-- TASKS — Project 6: Security Audit Q2 (8 tasks)
-- ============================================================
INSERT INTO tasks (title, description, project_id, assigned_to, created_by, status, priority, due_date, ai_suggested_priority) VALUES
...
('Dependency audit (npm + pip)',         'Snyk scan all repos. Patch all HIGH/CRITICAL CVEs within 7 days.',               6, 6, 1, 'in_progress', 'critical', '2026-05-15', 'critical'),
...
-- ============================================================
-- COMMENTS (sample comments for realism)
-- ============================================================
...
(41, 5, 'ZAP found 3 HIGH severity issues: missing CSRF token on /api/auth/register, open redirect on /login?next=, and reflected XSS on the search param.'),
(41, 1, 'Prioritise the CSRF fix — that goes to production tonight. Open redirect and XSS by end of week.'),
(42, 6, 'Snyk found 12 critical CVEs in the dependencies. Most are in old lodash and axios versions. Updating now.');
